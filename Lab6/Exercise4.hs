...
genNumberList = listOf genNumber `suchThat` (\x -> length x > 0)
...
--PART A: 
--Every element "x" in the domain must be in relation (x,something)
...
isSerial domainA rl = if list2set (domainA) == list2set(nub(map(fst) rl)) then True else False
...
isReflexive domainA rl = if list2set(filter (`inSet`  (list2set(map(createReflexive) domainA)) ) rl) == (list2set(map(createReflexive) domainA)) then True else False
...
isIrReflexive domainA rl = if not(isReflexive domainA rl) then True else False

isSymmetric ::(Eq a, Ord a) => Rel a -> Bool
isSymmetric rl =  if subSet (list2set (symClos rl)) (list2set (rl)) then True else False
...
lengthProperty domainA rl = if (length domainA) <= (length rl) then True else False

--PART C
funcR :: Int -> [Int] -> [Int]
funcR n y =map (mod n) y
...
genN = arbitrary::Gen Int
genListN = arbitrary :: Gen [Int]
